...
import (
	...
	// These api helpers are not shipped with the public repos.
	//TODO apihelpers "k8s.io/kubernetes/pkg/apis/flowcontrol/util"
	//TODO "k8s.io/api/core/helper"
	//TODO storageutil "k8s.io/kubernetes/pkg/apis/storage/util"
	//TODO "k8s.io/kubernetes/pkg/util/node"
)
...
func printReplicationController(obj *api.ReplicationController, options printers.GenerateOptions) ([]metav1.TableRow, error) {
	...
	row.Cells = append(row.Cells, obj.Name, desiredReplicas, int64(currentReplicas), int64(readyReplicas), translateTimestampSince(obj.CreationTimestamp))
	if options.Wide {
...
func printCSIStorageCapacity(obj *storagev1alpha1.CSIStorageCapacity, options printers.GenerateOptions) ([]metav1.TableRow, error) {
	row := metav1.TableRow{
		Object: runtime.RawExtension{Object: obj},
	}
...
func printCSIStorageCapacityList(list *storagev1alpha1.CSIStorageCapacityList, options printers.GenerateOptions) ([]metav1.TableRow, error) {
	rows := make([]metav1.TableRow, 0, len(list.Items))
	for i := range list.Items {
...
func printPersistentVolume(obj *api.PersistentVolume, options printers.GenerateOptions) ([]metav1.TableRow, error) {
	...
	row.Cells = append(row.Cells, obj.Name, aSize, modesStr, reclaimPolicyStr,
		string(phase), claimRefUID, GetPersistentVolumeClass(obj),
...
func printPersistentVolumeClaim(obj *api.PersistentVolumeClaim, options printers.GenerateOptions) ([]metav1.TableRow, error) {
	...
	row.Cells = append(row.Cells, obj.Name, string(phase), obj.Spec.VolumeName, capacity, accessModes,
		GetPersistentVolumeClaimClass(obj), translateTimestampSince(obj.CreationTimestamp), volumeMode)
	return []metav1.TableRow{row}, nil
}
...
func printPodSecurityPolicy(obj *policyv1beta1.PodSecurityPolicy, options printers.GenerateOptions) ([]metav1.TableRow, error) {
	row := metav1.TableRow{
		Object: runtime.RawExtension{Object: obj},
	}
...
func printPodSecurityPolicyList(list *policyv1beta1.PodSecurityPolicyList, options printers.GenerateOptions) ([]metav1.TableRow, error) {
	rows := make([]metav1.TableRow, 0, len(list.Items))
	for i := range list.Items {
